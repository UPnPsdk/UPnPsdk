...
LD_LIBRARY_PATH=build/Apps/api_calls Apps/bin/api_calls_pre_fork-psh
echo -e "Finished: test executable 'api_calls_pre_fork-psh' with UPnPsdk compatible library.\n"
...
if [ -f build/lib/libupnpsdk-pupnp.so.17 ]; then
    ...
    echo -e "Finished: test executable 'api_calls-psh' with UPnPsdk_compatible library: finished.\n"
fi
