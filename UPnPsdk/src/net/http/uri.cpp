...
void CUri::CAuthority::construct_from(std::string_view a_uri_sv) {
    ...
    // Here we have the authority_sv string with separator "//" or an empty one
    // representing an undefined authority. Having authority_sv only "//" means
    // authority is empty. Construct subcomponents.
...
void CUri::CAuthority::construct_scheme_file_from(std::string_view a_uri_sv) {
    ...
    if ((pos = a_uri_sv.find("//")) != npos && pos + 2 < a_uri_sv.size())
        // If there is at least one more any character (e.g. "//[") then an
...
std::string CUri::str() const {
    return (scheme.str() + ':' +
            ...
            (query.state() == STATE::avail ? "?" : "") +
            (query.state() == STATE::avail ? query.str() : "") +
            (fragment.state() == STATE::avail ? "#" : "") +
