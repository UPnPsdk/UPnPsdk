...
// Redistribution only with this Copyright remark. Last modified: 2025-11-13
...
TEST(CUriTestSuite, uri_host_scheme_file) {
...
TEST(CUriTestSuite, uri_remove_dot_segments_copy) {
    char uri1[]{"/foo-path/../bar"};
    std::string uriObj{uri1, strlen(uri1)};
    remove_dot_segments(uriObj);
    uriObj.copy(uri1, uriObj.size());
    ...
    char uri2[]{"/foo/bar"};
    uriObj = std::string(uri2, strlen(uri2));
    remove_dot_segments(uriObj);
    uriObj.copy(uri2, uriObj.size());
    uri2[uriObj.size()] = '\0';
...
// remove_dot_segments() function: tests from the uri module
// =========================================================
class RemoveDotSegmentsPTestSuite
    : public ::testing::TestWithParam<
          //           uri_path,         result
