...
TEST(UpnpFileInfoTestSuite, new_and_verify_struct_and_delete) {
    ...
    EXPECT_EQ(UpnpString_get_Length(f.info->m_Os), (size_t)0);
...
TEST(UpnpFileInfoDeathTest, set_get_file_length) {
    ...
    EXPECT_EQ(UpnpFileInfo_get_FileLength(f.info), (off_t)0);

    EXPECT_EQ(UpnpFileInfo_set_FileLength(f.info, -1), 1);
    EXPECT_EQ(UpnpFileInfo_get_FileLength(f.info), (off_t)-1);

    EXPECT_EQ(UpnpFileInfo_set_FileLength(f.info, 65536), 1);
    EXPECT_EQ(UpnpFileInfo_get_FileLength(f.info), (off_t)65536);
    ...
    if (old_code) {
        ...
        EXPECT_DEATH(UpnpFileInfo_set_FileLength(nullptr, 0), ".*");

    } else {

        // This expects NO segfault.
        ASSERT_EXIT((UpnpFileInfo_set_FileLength(nullptr, 0), exit(0)),
                    ExitedWithCode(0), ".*");
        int ret_file_length{0xAA5}; // 2725
        ret_file_length = UpnpFileInfo_set_FileLength(nullptr, 0);
    ...
    if (old_code) {
        std::cout << CYEL "[ BUGFIX   ]" CRES
                  << " UpnpFileInfo_get_FileLength called with nullptr must "
                     "not segfault.\n";
        // This expects segfault.
        EXPECT_DEATH(UpnpFileInfo_get_FileLength(nullptr), ".*");

    } else {

        // This expects NO segfault.
        ASSERT_EXIT((UpnpFileInfo_get_FileLength(nullptr), exit(0)),
                    ExitedWithCode(0), ".*");
        off_t file_length{(off_t)0xAA5555AA};
        file_length = UpnpFileInfo_get_FileLength(nullptr);
...
TEST(UpnpFileInfoDeathTest, set_get_last_modified) {
    ...
    EXPECT_EQ(UpnpFileInfo_get_LastModified(f.info), (time_t)0);

    EXPECT_EQ(UpnpFileInfo_set_LastModified(f.info, -1), 1);
    EXPECT_EQ(UpnpFileInfo_get_LastModified(f.info), (time_t)-1);

    EXPECT_EQ(UpnpFileInfo_set_LastModified(f.info, 75904), 1);
    EXPECT_EQ(UpnpFileInfo_get_LastModified(f.info), (time_t)75904);
    ...
    if (old_code) {
        std::cout
            ...
            << " UpnpFileInfo_set_LastModified() called with nullptr must "
               "not segfault.\n";
        // This expects segfault.
        EXPECT_DEATH(UpnpFileInfo_set_LastModified(nullptr, 77), ".*");

    } else {

        // This expects NO segfault.
        ASSERT_EXIT((UpnpFileInfo_set_LastModified(nullptr, 0), exit(0)),
                    ExitedWithCode(0), ".*");
    ...
    if (old_code) {
        std::cout
            ...
            << " UpnpFileInfo_get_LastModified() called with nullptr must "
               "not segfault.\n";
        // This expects segfault.
        EXPECT_DEATH(UpnpFileInfo_get_LastModified(nullptr), ".*");

    } else {

        // This expects NO segfault.
        ASSERT_EXIT((UpnpFileInfo_get_LastModified(nullptr), exit(0)),
                    ExitedWithCode(0), ".*");
        time_t ret_last_modified{0xAAAA};
        ret_last_modified = UpnpFileInfo_get_LastModified(nullptr);
    ...
    if (old_code) {
        std::cout << CYEL "[ BUGFIX   ]" CRES
                  << " UpnpFileInfo_set_IsDirectory() called with nullptr must "
                     "not segfault.\n";
        // This expects segfault.
        EXPECT_DEATH(UpnpFileInfo_set_IsDirectory(nullptr, false), ".*");

    } else {

        // This expects NO segfault.
        ASSERT_EXIT((UpnpFileInfo_set_IsDirectory(nullptr, false), exit(0)),
                    ExitedWithCode(0), ".*");
        int ret_is_directory{0x55AA5555};
        ret_is_directory = UpnpFileInfo_set_IsDirectory(nullptr, false);
    ...
    if (old_code) {
        std::cout << CYEL "[ BUGFIX   ]" CRES
                  << " UpnpFileInfo_get_IsDirectory() called with nullptr must "
                     "not segfault.\n";
        // This expects segfault.
        EXPECT_DEATH(UpnpFileInfo_get_IsDirectory(nullptr), ".*");

    } else {

        // This expects NO segfault.
        ASSERT_EXIT((UpnpFileInfo_get_IsDirectory(nullptr), exit(0)),
                    ExitedWithCode(0), ".*");
        int ret_is_directory{0x5555};
        ret_is_directory = UpnpFileInfo_get_IsDirectory(nullptr);
        EXPECT_EQ(ret_is_directory, 0);
    }
}
    ...
    if (old_code) {
        std::cout << CYEL "[ BUGFIX   ]" CRES
                  << " UpnpFileInfo_set_IsReadable() called with nullptr must "
                     "not segfault.\n";
        // This expects segfault.
        EXPECT_DEATH(UpnpFileInfo_set_IsReadable(nullptr, false), ".*");

    } else {

        // This expects NO segfault.
        ASSERT_EXIT((UpnpFileInfo_set_IsReadable(nullptr, false), exit(0)),
                    ExitedWithCode(0), ".*");
    ...
    if (old_code) {
        std::cout << CYEL "[ BUGFIX   ]" CRES
                  << " UpnpFileInfo_get_IsReadable() called with nullptr must "
                     "not segfault.\n";
        // This expects segfault.
        EXPECT_DEATH(UpnpFileInfo_get_IsReadable(nullptr), ".*");

    } else {

        // This expects NO segfault.
        ASSERT_EXIT((UpnpFileInfo_get_IsReadable(nullptr), exit(0)),
                    ExitedWithCode(0), ".*");
        int ret_is_readable{0xAA55};
        ret_is_readable = UpnpFileInfo_get_IsReadable(nullptr);
        EXPECT_EQ(ret_is_readable, 0);
    }
}
    ...
    if (old_code) {
        std::cout << CYEL "[ BUGFIX   ]" CRES
                  << " UpnpFileInfo_get_ContentType called with nullptr must "
                     "not segfault.\n";
        // This expects segfault.
        EXPECT_DEATH(UpnpFileInfo_get_ContentType(nullptr), ".*");

    } else {

        // This expects NO segfault.
        ASSERT_EXIT((UpnpFileInfo_get_ContentType(nullptr), exit(0)),
                    ExitedWithCode(0), ".*");
    ...
    if (old_code) {
        std::cout << CYEL "[ BUGFIX   ]" CRES
                  << " UpnpFileInfo_set_ContentType() called with nullptr must "
                     "not segfault.\n";
        // This expects segfault.
        EXPECT_DEATH(UpnpFileInfo_set_ContentType(nullptr, "segfault"), ".*");

    } else {

        // This expects NO segfault.
        ASSERT_EXIT(
            (UpnpFileInfo_set_ContentType(nullptr, "segfault"), exit(0)),
            ExitedWithCode(0), ".*");
        int ret_set_ContentType{0xAA55A};
        ret_set_ContentType = UpnpFileInfo_set_ContentType(nullptr, "segfault");
    ...
    if (old_code) {
        std::cout
            ...
            << " UpnpFileInfo_get_ContentType_cstr() called with nullptr must "
               "not segfault.\n";
        // This expects segfault.
        EXPECT_DEATH(UpnpFileInfo_get_ContentType_cstr(nullptr), ".*");

    } else {

        // This expects NO segfault.
        ASSERT_EXIT((UpnpFileInfo_get_ContentType_cstr(nullptr), exit(0)),
                    ExitedWithCode(0), ".*");
        const char* ret_get_ContentType_cstr{""};
        ret_get_ContentType_cstr = UpnpFileInfo_get_ContentType_cstr(nullptr);
...
TEST(UpnpFileInfoDeathTest, set_get_extra_headers_list) {
    ...
    if (old_code) {
        ...
        // This expects segfault.
        EXPECT_DEATH(UpnpFileInfo_set_ExtraHeadersList(f.info, nullptr), ".*");

    } else {

        // This expects NO segfault.
        ASSERT_EXIT(
            (UpnpFileInfo_set_ExtraHeadersList(f.info, nullptr), exit(0)),
            ExitedWithCode(0), ".*");
...
// ::UPnPsdk::SSockaddr to the function UpnpFileInfo_set_CtrlPtIPAddr()
// as argument. This test is the reason to reassign the structure
// ::UPnPsdk::SSockaddr. --Ingo
    ...
    if (old_code) {
        std::cout << CYEL "[ BUGFIX   ]" CRES
                  << " UpnpFileInfo_set_CtrlPtIPAddr() called with a nullptr "
                     "must not segfault.\n";
        // This expects segfault.
        EXPECT_DEATH(UpnpFileInfo_set_CtrlPtIPAddr(nullptr, &ss), ".*");

    } else {

        // This expects NO segfault.
        ASSERT_EXIT((UpnpFileInfo_set_CtrlPtIPAddr(nullptr, &ss), exit(0)),
                    ExitedWithCode(0), ".*");
        int ret_set_CtrlPtIPAddr{0x55A55A};
        ret_set_CtrlPtIPAddr = UpnpFileInfo_set_CtrlPtIPAddr(nullptr, &ss);
...
TEST(UpnpFileInfoDeathTest,
    ...
    if (old_code) {
        ...
        // This expects segfault.
        EXPECT_DEATH(UpnpFileInfo_set_CtrlPtIPAddr(f.info, nullptr), ".*");

    } else {

        // This expects NO segfault.
        ASSERT_EXIT((UpnpFileInfo_set_CtrlPtIPAddr(f.info, nullptr), exit(0)),
                    ExitedWithCode(0), ".*");
        int ret_set_CtrlPtIPAddr{0x55A5AA};
        ret_set_CtrlPtIPAddr = UpnpFileInfo_set_CtrlPtIPAddr(f.info, nullptr);
        EXPECT_EQ(ret_set_CtrlPtIPAddr, 0);
    }
}
...
TEST(UpnpFileInfoTestSuite, UpnpFileInfo_get_CtrlPtIPAddr_with_nullptr) {
    ...
    // Test Unit
    saddr = UpnpFileInfo_get_CtrlPtIPAddr(nullptr);
...
TEST(UpnpFileInfoTestSuite, UpnpFileInfo_clear_CtrlPtIPAddr) {
    ...
    ASSERT_EQ(UpnpFileInfo_set_CtrlPtIPAddr(f.info, &ss), 1);

    // Test Unit
    UpnpFileInfo_clear_CtrlPtIPAddr(f.info);

    const ::sockaddr_storage* saddr = UpnpFileInfo_get_CtrlPtIPAddr(f.info);
    ...
    if (old_code) {
        std::cout << CYEL "[ BUGFIX   ]" CRES
                  << " UpnpFileInfo_clear_CtrlPtIPAddr() called with a nullptr "
                     "must not segfault.\n";
        // This expects segfault.
        EXPECT_DEATH(UpnpFileInfo_clear_CtrlPtIPAddr((UpnpFileInfo*)nullptr),
                     ".*");
        ...
        // This expects NO segfault.
        EXPECT_EXIT(
            (UpnpFileInfo_clear_CtrlPtIPAddr((UpnpFileInfo*)nullptr), exit(0)),
            ExitedWithCode(0), ".*");
    }
}
...
TEST(UpnpFileInfoDeathTest, UpnpFileInfo_set_get_Os) {
    ...
    if (old_code) {
        ...
        // This expects segfault.
        EXPECT_DEATH(UpnpFileInfo_set_Os(f.info, nullptr), ".*");

    } else {

        // This expects NO segfault.
        ASSERT_EXIT((UpnpFileInfo_set_Os(f.info, nullptr), exit(0)),
                    ExitedWithCode(0), ".*");
        int ret_set_Os{0x5AA555A5};
        ret_set_Os = UpnpFileInfo_set_Os(f.info, nullptr);
    ...
    if (old_code) {
        std::cout << CYEL "[ BUGFIX   ]" CRES
                  << " UpnpFileInfo_set_Os() called with a nullptr "
                     "to an UpnpString must not segfault.\n";
        // This expects segfault.
        EXPECT_DEATH(UpnpFileInfo_set_Os(nullptr, upnp.str), ".*");

    } else {

        // This expects NO segfault.
        ASSERT_EXIT((UpnpFileInfo_set_Os(nullptr, upnp.str), exit(0)),
                    ExitedWithCode(0), ".*");
        int ret_set_Os{0x5AAA55A5};
        ret_set_Os = UpnpFileInfo_set_Os(nullptr, upnp.str);
        EXPECT_EQ(ret_set_Os, 0);
    }
}
...
TEST(UpnpFileInfoDeathTest, UpnpFileInfo_get_Os_with_nullptr) {
    ...
    if (old_code) {
        ...
        // This expects segfault.
        EXPECT_DEATH(UpnpFileInfo_get_Os((UpnpFileInfo*)nullptr), ".*");

    } else {

        // This expects NO segfault.
        ASSERT_EXIT((UpnpFileInfo_get_Os((UpnpFileInfo*)nullptr), exit(0)),
                    ExitedWithCode(0), ".*");
        const UpnpString* ret_get_Os{(UpnpString*)0x5AAA5AA5};
        ret_get_Os = UpnpFileInfo_get_Os((UpnpFileInfo*)nullptr);
...
TEST(UpnpFileInfoDeathTest, UpnpFileInfo_get_Os_Length) {
    ...
    // Test Unit
    EXPECT_EQ(UpnpFileInfo_get_Os_Length(f.info), (size_t)11);
    ...
    if (old_code) {
        std::cout << CYEL "[ BUGFIX   ]" CRES
                  << " UpnpFileInfo_get_Os_Length() with a nullptr must not "
                     "segfault.\n";
        // This expects segfault.
        EXPECT_DEATH(UpnpFileInfo_get_Os_Length((UpnpFileInfo*)nullptr), ".*");

    } else {

        // This expects NO segfault.
        ASSERT_EXIT(
            (UpnpFileInfo_get_Os_Length((UpnpFileInfo*)nullptr), exit(0)),
            ExitedWithCode(0), ".*");
        size_t ret_get_Os_Length{4508046};
        ret_get_Os_Length = UpnpFileInfo_get_Os_Length((UpnpFileInfo*)nullptr);
    ...
    if (old_code) {
        std::cout << CYEL "[ BUGFIX   ]" CRES
                  << " UpnpFileInfo_get_Os_cstr() with a nullptr must not "
                     "segfault.\n";
        // This expects segfault.
        EXPECT_DEATH(UpnpFileInfo_get_Os_cstr((UpnpFileInfo*)nullptr), ".*");

    } else {

        // This expects NO segfault.
        ASSERT_EXIT((UpnpFileInfo_get_Os_cstr((UpnpFileInfo*)nullptr), exit(0)),
                    ExitedWithCode(0), ".*");
        const char* ret_get_Os_cstr{(const char*)0x4508046};
        ret_get_Os_cstr = UpnpFileInfo_get_Os_cstr((UpnpFileInfo*)nullptr);
...
TEST(UpnpFileInfoDeathTest, UpnpFileInfo_strcpy_Os) {
    ...
    if (old_code) {
        ...
        // This expects segfault.
        EXPECT_DEATH(UpnpFileInfo_strcpy_Os((UpnpFileInfo*)nullptr, str), ".*");

    } else {

        // This expects NO segfault.
        ASSERT_EXIT(
            (UpnpFileInfo_strcpy_Os((UpnpFileInfo*)nullptr, str), exit(0)),
            ExitedWithCode(0), ".*");
        int ret_strcpy_Os{0x4F508046};
        ret_strcpy_Os = UpnpFileInfo_strcpy_Os((UpnpFileInfo*)nullptr, str);
...
#else
        // This expects segfault.
        EXPECT_DEATH(UpnpFileInfo_strcpy_Os(f.info, nullptr), ".*");
#endif

    } else {

        // This expects NO segfault.
        ASSERT_EXIT((UpnpFileInfo_strcpy_Os(f.info, nullptr), exit(0)),
                    ExitedWithCode(0), ".*");
        int ret_strcpy_Os{0x4FC08046};
        ret_strcpy_Os = UpnpFileInfo_strcpy_Os(f.info, nullptr);
        EXPECT_FALSE(ret_strcpy_Os);
    }
}
...
TEST(UpnpFileInfoDeathTest, UpnpFileInfo_strncpy_Os) {
    ...
    EXPECT_TRUE(UpnpFileInfo_strncpy_Os(f.info, str, (size_t)-1));
    EXPECT_STREQ(UpnpFileInfo_get_Os_cstr(f.info), "Hello world");
    ...
    if (old_code) {
        ...
        // This expects segfault.
        EXPECT_DEATH(UpnpFileInfo_strncpy_Os((UpnpFileInfo*)nullptr, str, 11),
                     ".*");
        ...
        // This expects NO segfault.
        ASSERT_EXIT(
            (UpnpFileInfo_strncpy_Os((UpnpFileInfo*)nullptr, str, 11), exit(0)),
            ExitedWithCode(0), ".*");
        int ret_strncpy_Os{0x4FD08046};
        ret_strncpy_Os =
            UpnpFileInfo_strncpy_Os((UpnpFileInfo*)nullptr, str, 11);
    ...
    if (old_code) {
        std::cout << CYEL "[ BUGFIX   ]" CRES
                  << " UpnpFileInfo_strncpy_Os() with a nullptr to the string "
                     "must not segfault.\n";
        // This expects segfault.
        EXPECT_DEATH(UpnpFileInfo_strncpy_Os(f.info, (const char*)nullptr, 11),
                     ".*");

    } else {

        // This expects NO segfault.
        ASSERT_EXIT((UpnpFileInfo_strncpy_Os(f.info, (const char*)nullptr, 11),
                     exit(0)),
                    ExitedWithCode(0), ".*");
        int ret_strncpy_Os{0x4CC08046};
        ret_strncpy_Os =
            UpnpFileInfo_strncpy_Os(f.info, (const char*)nullptr, 11);
        EXPECT_FALSE(ret_strncpy_Os);
    }
}
...
TEST(UpnpFileInfoDeathTest, UpnpFileInfo_clear_Os) {
    ...
    if (old_code) {
        ...
        // This expects segfault.
        EXPECT_DEATH(UpnpFileInfo_clear_Os((UpnpFileInfo*)nullptr), ".*");

    } else {

        // This expects NO segfault.
        EXPECT_EXIT((UpnpFileInfo_clear_Os((UpnpFileInfo*)nullptr), exit(0)),
                    ExitedWithCode(0), ".*");
    }
}
